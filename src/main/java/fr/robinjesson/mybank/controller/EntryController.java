...
public class EntryController {
    ...
    public ResponseEntity<?> addEntry(@RequestBody EntryRequest req) {
        try {
            ...
            return new ResponseEntity<>("No account found with id " + req.getAccountId() + ".", HttpStatus.NOT_FOUND);
        }
        catch(IllegalArgumentException e) {
            return new ResponseEntity("An error occured while adding a new entry", HttpStatus.BAD_REQUEST);
        }
    }
